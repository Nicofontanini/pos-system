...
function logEmployeeAction(action) {
  ...
  if (!employeeName) {
    showNotification('Por favor ingrese el nombre del empleado', 'warning');
    return;
  }
  ...
  fetch('/log-employee', {
    ...
    .then(data => {
      if (data.success) {
        showNotification(`${action === 'ingreso' ? 'Ingreso' : 'Egreso'} registrado exitosamente para ${employeeName}`, 'success');
    ...
    .catch(error => {
      ...
      showNotification('Error al registrar la acción', 'error');
    });
}
